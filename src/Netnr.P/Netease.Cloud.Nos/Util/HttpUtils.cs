...
namespace Netease.Cloud.NOS.Util
{
    internal static class HttpUtils
    {
        ...
        private const string URL_ALLOWED_CHARS = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789-_.~";
        private static IDictionary<string, string> _mimeDict = new Dictionary<string, string>();
        public const string MIMETYPE_OCTET_STREAM = "application/octet-stream";

        /// <summary>
        /// 连接查询字符串
        /// </summary>
        /// <param name="parameters">可设置或检索的查询参数键值对</param>
        ...
        public static string EncodeUri(string uri, string charset)
        {
    ...
    public static class FileContentType
    {
        ...
        public static string GetMimeType(string key, string file)
        {
            if (file != null)
            {
                if (!file.StartsWith("."))
                ...
                return _mappings.TryGetValue(file, out mime) ? mime : MIMETYPE_OCTET_STREAM;
            }
            else if (key != null)
            {
                if (!key.StartsWith("."))
                ...
                return _mappings.TryGetValue(key, out mime) ? mime : MIMETYPE_OCTET_STREAM;
            }
