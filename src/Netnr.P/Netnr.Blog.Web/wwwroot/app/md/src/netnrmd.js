...
(function (window) {
    ...
    netnrmd.fn = netnrmd.prototype = {
        ...
        isDark: function () {
            return this.obj.editor.classList.contains("netnrmd-dark")
        },

        //切换主题
        toggleTheme: function (theme) {
            var obj = this.obj;
            var isDark = theme == null ? !obj.editor.classList.contains("netnrmd-dark") : theme == "dark";
            if (isDark) {
                obj.editor.classList.add("netnrmd-dark");
            } else {
                obj.editor.classList.remove("netnrmd-dark")
            }

            for (var o in this) {
                var oi = this[o];
                if (typeof oi == "object" && oi.nodeType == 1 && oi.classList.contains("netnrmd-popup")) {
                    if (isDark) {
                        oi.classList.add("netnrmd-dark");
                    } else {
                        oi.classList.remove("netnrmd-dark");
                    }
                }
            }
            ...
            case "emoji":
                {
                    if (!that.emojipopup) {
                        ...
                        htm.push("<div>")
                        //弹出
                        that.emojipopup = netnrmd.popup("表情（按住 Ctrl 连选）", htm.join(''), that.isDark());
                        //选择表情
     ...
     * @param {any} isdark 暗黑主题
     */
    netnrmd.popup = function (title, content, isdark) {
        ...
        var pp = netnrmd.createDom('div', 'netnrmd-popup' + (isdark ? ' netnrmd-dark' : ''), htm.join(''));
