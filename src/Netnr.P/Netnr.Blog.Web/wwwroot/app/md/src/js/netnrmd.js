...
var netnrmd = {
    ...
    popup: function (title, content) {
        ...
        var pp = this.createDom('div', 'netnrmd-popup', htm.join(''));
    ...
    toc: function (ops) {
        ...
        var menus = ops.domMarkdown.querySelectorAll("h1,h2,h3,h4,h5,h6");
        ...
        if (menus.length > 0) {
            ...
            var domMarkdownContainer = ops.domMarkdown.parentElement;
            domMarkdownContainer.classList.add('markdown-container');

            var domToc = document.createElement('div');
     ...
     * @param {any} callback
     */
    readyPackage: function (name, callback) {
        if (!window[name]) {
            var src = netnrmd.resource[name];
            if (window["require"]) {
                window["require"]([src], (obj) => {
                    window[name] = obj;
                    callback();
                });
            } else {
                netnrmd.getScript(src, callback)
            }
        } else {
            callback();
        }
    ...
    toolbarBuildIcon: function (itemIcon) {
        ...
        var li = `<li class="${itemIcon.class || ''}" data-cmd="${itemIcon.cmd}" title="${tooltip}">
    ...
    toolbarIcons: [
            ...
            title: '关于', cmd: 'about', svg: '<path d="m10.277 5.433-4.031.505-.145.67.794.145c.516.123.619.309.505.824L6.101 13.68c-.34 1.578.186 2.32 1.423 2.32.959 0 2.072-.443 2.577-1.052l.155-.732c-.35.31-.866.434-1.206.434-.485 0-.66-.34-.536-.939l1.763-8.278zm.122-3.673a1.76 1.76 0 1 1-3.52 0 1.76 1.76 0 0 1 3.52 0z"/>', action: function (that) {
                if (!that.domAbout) {
                    var html = [
                        ...
                        'Copyright © [netnr](https://www.netnr.com)',
        ...
        { title: '主题', cmd: 'theme', class: 'float-right', svg: '<path d="M8 16A8 8 0 1 0 8 0a8 8 0 0 0 0 16zm6.43-5.228a7.025 7.025 0 0 1-3.658 3.658l-1.115-2.788a4.015 4.015 0 0 0 1.985-1.985l2.788 1.115zM5.228 14.43a7.025 7.025 0 0 1-3.658-3.658l2.788-1.115a4.015 4.015 0 0 0 1.985 1.985L5.228 14.43zm9.202-9.202-2.788 1.115a4.015 4.015 0 0 0-1.985-1.985l1.115-2.788a7.025 7.025 0 0 1 3.658 3.658zm-8.087-.87a4.015 4.015 0 0 0-1.985 1.985L1.57 5.228A7.025 7.025 0 0 1 5.228 1.57l1.115 2.788zM8 11a3 3 0 1 1 0-6 3 3 0 0 1 0 6z"/>', action: function (that) { that.toggleTheme() } }
    ],
