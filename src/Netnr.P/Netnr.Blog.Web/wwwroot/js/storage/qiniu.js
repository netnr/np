var nrObj = {
    ...
    init: () => new Promise((resolve, reject) => {
        ...
        fetch("/Storage/QiniuAPI").then(x => x.json()).then(res => {
            nrObj.lsLoad(); //本地配置

            //载入桶
            res.Bucket.split(',').forEach(item => {
                nrObj.domSeBucket.options.add(new Option(item, item));
            });
    ...
    getSignature: function (options) {
        //1.生成待签名的原始字符串，拼接 Method、Path、query、Host、Header、Body

        //req.Method + " " + req.Path + ? + req.query
        //Host: + " " + req.Host
        //Content-Type: +" " + req.Content-Type //如果设置Content-Type、X-Qiniu-<key>
        //
        //
        //req.Body //如果设置Body，并且设置Content-Type不为"application/octet-stream"类型
        var signingArr = [
            options.method || "GET",
            ' ',
            options.path,
            options.query,
            '\nHost: ',
            options.host.split('://').pop(),
            '\n'
        ];
        options.headers = options.headers || {};
        var ct = options.headers['Content-Type'];
        if (ct != null) {
            signingArr.push(`Content-Type: ${ct}`);
        }
        //遍历 headers
        var xQiniuKeys = [];
        for (var key in options.headers) {
            if (key.startsWith('X-Qiniu-')) {
                xQiniuKeys.push({ key: key.split('-').pop().toLowerCase(), value: options.headers[key] });
            }
        }
        if (xQiniuKeys.length > 0) {
            xQiniuKeys.sort((a, b) => a.key.localeCompare(b.key));
            xQiniuKeys.forEach(x => signingArr.push(`\n${x.key}: ${x.value}`));
        }
        signingArr.push('\n\n');
        if (options.body != null && ct != 'application/octet-stream') {
            signingArr.push(options.body);
        }
        var signingStr = signingArr.join('');
        console.log(signingStr);
        //2. 使用 SecertKey 对上一步生成的原始字符串计算 HMAC-SHA1 签名
        var sign = CryptoJS.HmacSHA1(signingStr, nrObj.config.SecretKey).toString();
        //3. 对签名进行URL 安全的 Base64 编码
        var encodedSign = btoa(sign).replaceAll('+', '-').replaceAll('/', '_');
        //4. 将 AccessKey 和 encodedSign 用英文符号 : 连接起来
        var accessToken = nrObj.config.AccessKey + ':' + encodedSign;
    ...
    getBucket: (options) => new Promise((resolve, reject) => {
        ...
        var signObj = {
            method: 'GET',
            path: `/list`,
            query: `?${nrObj.parameterJoin(ops)}`,
            host: nrObj.rsfHost,
            headers: {
                'Content-Type': 'application/x-www-form-urlencoded',
            }
        }
        signObj.headers['Authorization'] = `Qiniu ${nrObj.getSignature(signObj)}`;
        fetch(`https://${signObj.host}${signObj.path}${signObj.query}`, {
    ...
    parameterJoin: (obj, lowerCaseKey) => {
        var arr = [];
        for (var key in obj) {
            var skey = lowerCaseKey ? key.toLowerCase() : key;
            arr.push(skey + '=' + encodeURIComponent(obj[key]))
        }
        return arr.join('&');
    },
    modal: (options) => {
