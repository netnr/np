var nrObj = {
    ...
    init: () => new Promise((resolve, reject) => {
        ...
        nrObj.domSeBucket.addEventListener('change', function () {
            ...
            nrObj.nos.options.Bucket = this.value;
            nrObj.lsSave();
            nrObj.load();
        }, false);
        //域名
        ...
        nrObj.domTxtDomain.addEventListener('input', function () {
            ...
            nrObj.nos.options.Domain = this.value;
            nrObj.lsSave();
        }, false);

        fetch("/Storage/NetEasyAPI").then(x => x.json()).then(res => {
            nrObj.lsLoad(); //本地配置
            ...
            //载入配置、初始化
            nrObj.config.accessKey = res.AccessKeyId;
            nrObj.config.accessSecret = res.AccessKeySecret;
            nrObj.config.endpoint = res.Endpoint;
            nrObj.config.bucket = res.Bucket;

            nrObj.nos = new NOS();
            nrObj.nos.setAccessId(nrObj.config.accessKey);
            nrObj.nos.setSecretKey(nrObj.config.accessSecret);
            nrObj.nos.setEndpoint(nrObj.config.endpoint);
            nrObj.nos.setProtocol('https');
            nrObj.nos.setPort('443');
    ...
    getBucket: (options) => new Promise((resolve, reject) => {
        nrObj.nos.list_objects({ bucket: nrObj.config.bucket }, function (err, data) {
    ...
    putObject: (options) => new Promise((resolve, reject) => {
        nrObj.nos.putObject(Object.assign({
    ...
    uploadFile: (options) => new Promise((resolve, reject) => {
        ...
        nrObj.nos.uploadFile(Object.assign({
            Bucket: nrObj.config.bucket,
            Region: nrObj.config.Region,
            Key: '',
            Body: null,
    ...
    deleteMultipleObject: (keys) => new Promise((resolve, reject) => {
        ...
        nrObj.nos.deleteMultipleObject({
            Bucket: nrObj.config.bucket,
            Region: nrObj.config.Region,
            Objects: keys
        }, function (err, data) {
    ...
    getObjectUrl: (options) => new Promise((resolve, reject) => {
        nrObj.nos.getObjectUrl(Object.assign({
            Bucket: nrObj.config.bucket,
            Region: nrObj.config.Region,
            Key: '',
            Sign: false
