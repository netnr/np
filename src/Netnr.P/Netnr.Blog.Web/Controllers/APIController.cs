...
namespace Netnr.Blog.Web.Controllers
{
    ...
    public partial class APIController : ControllerBase
    {
        ...
        {
            var vm = new ResultVM();
            if (!string.IsNullOrWhiteSpace(token) && token.Length > 5 && AppTo.GetValue("ProgramParameters:GlobalKey").Contains(token))
            {
        ...
        internal static async Task<ResultVM> UploadCheck(IFormFile file, byte[] content, string ext, string subdir)
        {
            ...
            if (file == null || ParsingTo.IsRiskFile(file.FileName))
            {
                vm.Set(RCodeTypes.refuse);
                vm.Msg = "File not supported";
            }
            else
            {
        ...
        public async Task<ResultVM> Upload(IFormFile file, [FromForm] string subdir = "/static")
        {
            ...
            try
            {
                if (AppTo.GetValue<bool?>("ProgramParameters:DisableDatabaseWrite") != true && !HttpContext.User.Identity.IsAuthenticated)
                {
                    vm.Set(RCodeTypes.unauthorized);
                }
                else if (file == null)
        ...
        /// <returns></returns>
        [HttpGet]
        [Route("api/v1/Clock/{timezone:int?}")]
        ...
        /// <returns></returns>
        [HttpPost]
        public ResultVM OCRForType(IFormFile file, [FromForm] string type, [FromForm] string side, [FromForm] string API_KEY, [FromForm] string SECRET_KEY)
        {
            var vm = new ResultVM();
            try
            {
        ...
        /// <returns></returns>
        [HttpPost]
        public ResultVM NLP([FromForm] string type, [FromForm] string title, [FromForm] string content, [FromForm] int maxSummaryLen, [FromForm] string API_KEY, [FromForm] string SECRET_KEY)
        {
            var vm = new ResultVM();
            try
            {
        ...
        [HttpPatch]
        [HttpDelete]
        public async Task<IActionResult> Proxy(string url, string charset = "utf-8")
        {
            ...
            try
            {
                ...
                var keywordBlacklist = new List<string> { ".m3u8", ".ts", ".mpd", ".m4v" };
                ...
                else
                {
                    ...
                    var method = Request.Method;  // 获取请求方法
                    ...
                    {
                        if (!skipHeader.Contains(header.Key))
                        {
                    ...
                    if (method == "POST" || method == "PATCH" || method == "DELETE")
                    {
                        ...
                        {
                            // FormUrlEncodedContent 不允许键名重复
                            var args = string.Join('&', Request.Form.Select(x => $"{x.Key}={x.Value.ToString().ToUrlEncode()}"));
                            request.Content = new StringContent(args, encoding, contentType.Split(';')[0]);
                        }
                        else if (contentType.StartsWith("multipart/form-data"))
                        {
                            ...
                            foreach (var key in Request.Form.Keys)
                            {
                                ...
                                formContent.Add(new StringContent(stringValue), key);
                            }
                        ...
                        else
                        {
                            ...
                            var content = new StringContent(requestBody, encoding, contentType.Split(';')[0] ?? "text/plain");
                    ...
                    allHeaders.ForEach(h =>
                    {
                        ...
                        {
                            Response.Headers.Append(h.Key, h.Value.ToArray());
                        }
                    });
                    ...
                    return File(stream, response.Content.Headers.ContentType?.ToString() ?? outContentType);
                }
            }
            catch (Exception ex)
            {
