...
var ndkRequest = {
    ...
    statusInfo: {
        ...
        si: null,
        run: () => {
            ndkVary.domReqProgress.indeterminate = true;
            document.querySelector('sl-icon-button').classList.add('nrc-rotate-1');
        },
        stop: () => {
            ndkVary.domReqProgress.indeterminate = false;
        ...
        fetch(url, Object.assign({ method: "GET", signal: reqObj.abortCtrl.signal }, options)).then(resp => {
            ...
            if (res.code == 200) {
                ...
                if (sql) {
                    var msgHtml = "";
                    try {
                        msgHtml = JSON.stringify(JSON.parse(res.msg), null, 2);
                    } catch (error) {
                        msgHtml = res.msg;
                    }

                    ndkFunction.alert(`<sl-textarea rows="13"></sl-textarea><sl-divider></sl-divider><code contenteditable style="word-break:break-all">${sql.substring(0, 999)}</code>`, ndkI18n.lg.message);
                    ndkVary.domAlert.querySelector('sl-textarea').value = msgHtml;
                    reject(res);
                } else {
            ...
            ndkFunction.alert(`${setHost}${err}<sl-divider></sl-divider><code style="word-break:break-all">${url}</code>`, ndkI18n.lg.message);
            ndkRequest.statusInfo.queueRemove(reqObj);
            reject(err);
        });
    }),
                ...
                ndkStorage.cc([cobj.id, databaseName, '*']).then(cout => {
                    if (cout != null) {
                        ...
                        if (filterSNTN != null && filterSNTN != "") {
                            ...
                            //获取左侧所有表
                            var arows = nrGrid.getAllRows(ndkVary.gridOpsTable);
