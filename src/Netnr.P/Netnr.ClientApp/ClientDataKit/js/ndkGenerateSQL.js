...
-- show open tables where In_use > 0`);
            ...
            case "Oracle":
                switch (name) {
                    ...
                    case "Lock":
                        sqls.push(`-- 查看被锁表信息
SELECT se.sid, se.serial#, lo.oracle_username, lo.os_user_name, ao.object_name, lo.locked_mode FROM v$locked_object lo, dba_objects ao, v$session se WHERE ao.object_id = lo.object_id AND lo.session_id = se.sid;
...
WHERE A.SID IN (SELECT DISTINCT T2.SID FROM V$LOCKED_OBJECT T1, V$SESSION T2 WHERE T1.SESSION_ID = T2.SID) AND A.SQL_ADDRESS = C.ADDRESS (+)
ORDER BY C.PIECE;
`);
                        break;
                    case "TableSpace":
