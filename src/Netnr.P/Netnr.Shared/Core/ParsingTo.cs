...
public partial class ParsingTo
{
    ...
    /// <summary>
    /// 按字节大小分隔字符串（在90%-100% 优先按换行进行分隔）
    /// </summary>
    /// <param name="content"></param>
    /// <param name="byteSize">字节大小</param>
    /// <param name="encoding">编码，默认 UTF8</param>
    /// <returns></returns>
    public static List<string> SplitByByteSize(string content, int byteSize, Encoding encoding = null)
    {
        ...
        foreach (char c in content)
        {
            ...
            if (currentBytes + charBytes > byteSize)
            {
                ...
                // 在分隔长度的90%-100% 有换行，则优先根据换行拆分
                int lastNewlineIndex = part.LastIndexOf('\n');
                if (lastNewlineIndex != -1 && Convert.ToDouble(lastNewlineIndex) % Convert.ToDouble(part.Length) > 0.9)
                {
                    parts.Add(sb.ToString(0, lastNewlineIndex));

                    sb = new StringBuilder(part.Substring(lastNewlineIndex + 1));
                    currentBytes = (encoding ?? Encoding.UTF8).GetByteCount(sb.ToString());
                }
                else
                {
    ...
    /// <returns></returns>
    public static bool IsRiskFile(string filename)
    {
        var risk = false;
        if (string.IsNullOrWhiteSpace(filename) || filename.Length > 250 || filename.EndsWith(".") || filename.IndexOfAny(Path.GetInvalidFileNameChars()) >= 0)
        {
            risk = true;
        }
        else
    ...
    /// 风险扩展名
    /// </summary>
    private static readonly List<string> RiskExtensions =
    [
        ".exe",
        ".msi",
        ".bat",
