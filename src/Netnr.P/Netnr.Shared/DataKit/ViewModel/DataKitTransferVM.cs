...
/// 传递参数
/// </summary>
public partial class DataKitTransferVM
{
    /// <summary>
    /// 连接对象
    /// </summary>
    public class ConnectionInfo
    {
        ...
        public string DatabaseName
        {
            get
            {
                ...
                {
                    //默认数据库名
                    return DbHelper.DbConn(ConnectionType, ConnectionString).Database;
                }
                else
                {
            ...
            set
            {
                ConnectionString = DbHelper.SetConnDatabase(ConnectionType, ConnectionString, value);
                _DatabaseName = value;
            }
        }
    ...
    public class MigrateDatabase : MigrateBase
    {
        ...
        /// <returns></returns>
        public MigrateDataTable AsMigrateDataTable(bool isSimilar = true)
        {
            var mdb = this;
            var mdt = new MigrateDataTable().ToCopy(mdb);

            var readTables = DataKitTo.Init(mdt.ReadConnectionInfo).GetTable();
            var writeTables = DataKitTo.Init(mdt.WriteConnectionInfo).GetTable();
            ...
            if (readTables?.Count > 0 && writeTables?.Count > 0)
            {
                ...
                {
                    //读取库的表名 在 写入库
                    List<DataKitTableVM> listWriteTable = null;
