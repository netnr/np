...
namespace Netnr.SharedDataKit.Applications
{
    ...
    public class DataKitSQLServerService : IDataKitService
    {
        ...
        {
            db = new DbHelper(new Microsoft.Data.SqlClient.SqlConnection(connectionString = conn));
        }
        ...
        public DkDEI GetDEI()
        {
            ...
            var ds = db.SqlQuery(@"
                                SELECT
	                                SUBSTRING(@@VERSION , 1, CHARINDEX(')', @@VERSION, 1))+ ' ' + cast(SERVERPROPERTY('Edition') as varchar) AS DeiName ,
            ...
            {
                if (dt1.Columns.Contains(pi.Name))
                {
                    object val = dt1.Rows[0][pi.Name];
                    if (pi.Name == "DeiIgnoreCase")
                    {
                        val = val?.ToString() == "1";
                    }
                    pi.SetValue(mo, val);
