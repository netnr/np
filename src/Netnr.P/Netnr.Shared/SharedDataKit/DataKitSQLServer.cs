...
namespace Netnr.SharedDataKit
{
    ...
    public class DataKitSQLServer : IDataKit
    {
        ...
        public DataKitSQLServer(DbConnection dbConn)
        {
        ...
        /// <param name="databaseName"></param>
        /// <returns></returns>
        public string GetTableDDL(string tableName, string tableSchema, string databaseName = null)
        {
            if (string.IsNullOrWhiteSpace(databaseName))
            {
                databaseName = DefaultDatabaseName();
            ...
            var sql = Configs.GetTableDDLSQLServer(databaseName, tableSchema, tableName);
            var ds = db.SqlExecuteReader(sql);

            var ddl = ds.Item1.Tables[0].Rows[0][1].ToString();
            return ddl;
        }
        ...
        public Tuple<DataSet, DataSet, object> ExecuteSql(string sql, string databaseName = null)
        {
            ...
            var dbConn = (SqlConnection)db.Connection;
