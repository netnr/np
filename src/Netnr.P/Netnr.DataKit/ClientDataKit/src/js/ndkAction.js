...
<div class="view-cell-body" style="margin-top:1em;min-height:40vh"></div>`;
    ...
    themeGrid: function () {
        ...
        //选项卡（右侧主体）窗口 表格
        ndkVary.domTabGroupBody.querySelectorAll('.nr-grid-execute-sql').forEach(dom => {
            setTheme(dom);
        })
    ...
    size: function () {
        ...
        //选项卡 窗口 执行SQL
        ndkVary.domTabGroupBody.querySelectorAll('.nr-spliter-sql').forEach(node => {
        ...
        //选项卡 窗口 生成代码
        ndkVary.domTabGroupBody.querySelectorAll('.nr-spliter-code').forEach(node => {
            var pnode = node.parentElement;
            if (getComputedStyle(pnode).display != "none") {
                var gt = node.getBoundingClientRect().top + 5;
                node.style.height = `${vh - gt}px`;
