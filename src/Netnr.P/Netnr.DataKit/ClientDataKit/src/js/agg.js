...
var agg = {
    ...
    optionDef: ops => Object.assign({
        ...
        getMainMenuItems: function (params) {
            params.defaultItems.unshift("separator");
            params.defaultItems.unshift({
                name: ndkI18n.lg.copyTitle,
                icon: ndkVary.iconGrid("copy"),
                action: () => ndkFunction.clipboard(params.column.getId())
            });
     ...
     * @returns 
     */
    getSelectedOrRangeRows: function (gridOps) {
        var srows = gridOps.api.getSelectedRows(), crows = gridOps.api.getCellRanges();
        if (crows.length > 0) {
            var startRowIndex = crows[0].startRow.rowIndex,
                endRowIndex = crows[0].endRow.rowIndex;

            for (let rowIndex = startRowIndex; rowIndex <= endRowIndex; rowIndex++) {
                var row = gridOps.api.getDisplayedRowAtIndex(rowIndex).data;
                if (row) {
                    srows.push(row);                    
                }
            }
